            ...
            return (
              <a
                ...
                className={`flex flex-col gap-2 rounded-2xl px-5 py-4 transition hover:-translate-y-0.5 hover:shadow ${
                  isLinkedIn
                    ? "bg-[#f0f7ff] hover:bg-[#e3f0ff]"
                    : "bg-pale-gray hover:bg-charcoal/5"
                }`}
              >
                {item.description ? (
      ...
      return (
        <div
          ...
          className={`flex items-center justify-between gap-6 rounded-2xl border px-6 py-5 ${
            isLinkedIn
              ? "border-[#0a66c2]/15 bg-[#f0f7ff]"
              : "border-charcoal/8 bg-pale-gray"
          }`}
        >
        ...
        >
          {block.images.map((img: ProjectImageItem) => (
            <figure key={img.src} className="space-y-2">
              <div className="overflow-hidden rounded-[1.25rem] border border-white/80 bg-white shadow-soft">
                <Image
                  src={img.src}
                  alt={img.alt}
                  width={800}
              ...
              {img.caption ? (
                <figcaption className="text-xs leading-5 text-charcoal/60">
                  {img.caption}
                </figcaption>
              ) : null}
            </figure>
          ...
          <article className="space-y-8">
            <div className="surface-panel rounded-[2rem] p-8 shadow-soft md:p-10">
              <div className="space-y-5">
                ...
                <div className="space-y-4">
                  <div className="flex flex-wrap items-center gap-3">
                    {project.tags.filter((tag) => tag !== "Project").map((tag) => (
                      <span
                        key={tag}
                        className="inline-flex items-center rounded-full bg-[#d9f3c7] px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-charcoal"
                      >
                        {tag}
                      </span>
                    ))}
                  ...
                  <div className="flex flex-wrap gap-x-6 gap-y-1 text-[11px] font-semibold uppercase tracking-[0.22em] text-charcoal/55">
                ...
                <div className="overflow-hidden rounded-[1.6rem] bg-charcoal/6">
                  <Image
